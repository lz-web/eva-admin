<template>
  <div class="lin-table">
    ...
    >
      ...
      <el-table-column v-if="operate2.length > 0" label="操作" fixed="right" width="175">
        <template slot-scope="scope">
          <el-button
            v-for="(item, index) in operate2"
            v-show="(scope.row.user_status == '通过' && item.name == '停用') || (scope.row.user_status== '待审核' && item.name != '停用')"
            :type="item.type"
            plain
            :key="index"
            size="mini"
...
export default {
  ...
  methods: {
    ...
    // 行内 通过
    handlePass(_this, index, row) {
      _this.$emit('handlePass', { index, row })
    },
    // 行内 拒绝
    handleRefuse(_this, index, row) {
      _this.$emit('handleRefuse', { index, row })
    },
    // 行内 禁用
    handleStop(_this, index, row) {
      _this.$emit('handleStop', { index, row })
    },
    // 行内 查看详情
    ...
    // 行内 下载
    handleDownload(_this, index, row) {
      _this.$emit('handleDownload', { index, row })
    },
    // 多选-选中checkbox
